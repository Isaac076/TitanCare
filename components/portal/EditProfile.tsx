...
export function EditProfile({ patientId, currentName, currentPhoto, onSuccess, onClose }: Props) {
  ...
  const handleFileSelected = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    if (!file.type.startsWith('image/')) { setError(t('edit_error_type')); return }
  ...
  const handleSave = async () => {
    if (name.trim().length < 2) { setError(t('edit_error_short')); return }
    setLoading(true)
    setError('')
    try {
      ...
      if (photoFile) {
        ...
        const photoRes = await apiFetch('/api/patient/photo', { method: 'POST', body: formData })
        if (!photoRes.ok) {
          const data = await photoRes.json()
          throw new Error(data.error ?? t('edit_error_photo'))
        }
      ...
      if (!nameRes.ok) throw new Error(t('edit_error_name'))
      setSuccess(true)
      setTimeout(() => onSuccess(name.trim(), finalPhotoUrl), 1200)
    } catch (err: any) {
      setError(err.message ?? t('edit_error_generic'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 bg-navy-950/90 backdrop-blur-sm flex items-end sm:items-center justify-center p-4 animate-fade-in">
      <div className="w-full max-w-sm bg-navy-900 border border-white/10 rounded-[24px] p-6">
        <div className="flex items-center justify-between mb-6">
          <p className="text-white font-medium">{t('edit_title')}</p>
        ...
        {success ? (
          <div className="flex flex-col items-center py-6 gap-3">
            ...
            <p className="text-white font-medium">{t('edit_success')}</p>
          </div>
        ) : (
          <>
            ...
            <button onClick={handleSave} disabled={loading} className="w-full bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white rounded-[12px] py-3.5 flex items-center justify-center gap-2 font-medium text-[14px] transition-colors">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
              {loading ? t('edit_saving') : t('edit_save')}
            </button>
          </>
        )}
      </div>
